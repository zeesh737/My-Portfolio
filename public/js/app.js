...
const initializeProjectsFlow = () => {
  ...
  const projectsData = Array.from(projectsEl.querySelectorAll('.project-item')).map(item => {
    ...
    const detailsEl = item.querySelector('.project-details');
    if (detailsEl) {
      projectObj.details = {
        role: item.getAttribute('data-role') || '',
        duration: item.getAttribute('data-duration') || '',
        team: item.getAttribute('data-team') || '',
        description: detailsEl.querySelector('.description')?.innerHTML.trim() || '',
        tools: Array.from(detailsEl.querySelectorAll('.tools li')).map(li => li.innerHTML.trim()),
        responsibilities: Array.from(detailsEl.querySelectorAll('.responsibilities li')).map(li => li.innerHTML.trim())
      };
    }
...
const generateAndProcessResponse = async (command, displayText = command) => {
  // typed shortcuts that must not touch the intro screen
  const typed = command.trim().toLowerCase();
  if (typed === 'quick view') {
    chatInput.value = '';
    toggleSendButtonState();
    window.openCVQuickView();
    return;
  }
  if (typed === 'home') {
    goHome();
  ...
  if (intro && !intro.classList.contains('fade-out')) {
    ...
    setTimeout(() => {
      if (intro.classList.contains('fade-out')) intro.remove();
    }, 600);
  }
...
/* ==========================================================================
Keeps the "?quick_view=true" URL parameter in sync with the CV quick view popup.
========================================================================== */

const setQuickViewParam = (isOpen) => {
  try {
    const url = new URL(window.location.href);
    if (isOpen) {
      url.searchParams.set('quick_view', 'true');
    } else {
      url.searchParams.delete('quick_view');
    }
    window.history.replaceState(window.history.state, '', url.pathname + url.search + url.hash);
  } catch (e) {
    // history API not available (e.g. restricted environment): the popup still works
  }
};

/* ==========================================================================
Returns to the main (intro) screen and clears the conversation.
========================================================================== */

const goHome = () => {
  const contentArea = document.querySelector('.chat-area-content');
  if (!contentArea || !introScreenEl) return;
  Array.from(chatWindow.children).forEach(child => {
    if (child.id !== 'chat-avatar') child.remove();
  });
  currentProjectIndex = 0;
  if (chatInput) {
    chatInput.value = '';
  ...
  if (primaryNav) primaryNav.classList.remove('menu-active');
  document.body.classList.remove('open-menu');
  if (!introScreenEl.isConnected) {
    introScreenEl.classList.add('fade-out');
    contentArea.prepend(introScreenEl);
    void introScreenEl.offsetHeight;
  }
  introScreenEl.classList.remove('fade-out');
...
const renderOptions = (options, parentElement, isInsideCard = false) => {
  ...
  options.forEach((option, index) => {
    ...
    if (option.action) {
      ...
      // "cv-url" points to the CV set in <meta name="cv-url"> in index.html
      const isCV = option.link === 'cv-url';
      const cvMeta = document.querySelector('meta[name="cv-url"]');
      el.href = isCV ? (cvMeta ? cvMeta.getAttribute('content') : '#') : option.link;
      el.target = '_blank';
      el.rel = 'noopener';
      if (isCV) el.setAttribute('download', '');
    }
  ...
  if (message.isRich && message.type === 'single-project-card') {
    ...
    card.innerHTML = `
        ...
        <div class="details">
            ...
            ${project.details ? `
            <button type="button" class="btn btn-primary btn-secondary quick-view-btn">
                <span class="button-content">
                    <span>${isArabic ? 'نظرة سريعة' : 'View Details'}</span>
                    <span aria-hidden="true">${isArabic ? 'نظرة سريعة' : 'View Details'}</span>
                </span>
            </button>` : ''}
        </div>
    `;
    ...
    if (quickViewBtn) {
      quickViewBtn.onclick = () => openProjectDetails(project);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.openMasterModal = (project) => {
    ...
    area.innerHTML = '';
    modal.classList.remove('modal-details', 'modal-contact', 'modal-cv');
    if (project.mediaType === 'gallery') {
  ...
  // Modal For Project Quick View (role, team, tech stack, responsibilities)
  window.openProjectDetails = (project) => {
    ...
    const area = document.getElementById('modal-content-area');
    const t = isArabic
      ? { duration: 'المدة', role: 'الدور', team: 'حجم الفريق', tools: 'الأدوات والتقنيات', resp: 'المسؤوليات والإنجازات' }
      : { duration: 'Duration', role: 'Role', team: 'Team size', tools: 'Tools & Technologies', resp: 'Responsibilities & Achievements' };
    const metaItem = (icon, label, value) => value
      ? `<li><i class="${icon}"></i><span class="pd-label">${label}</span><strong>${value}</strong></li>`
      : '';
    window.currentSlideIndex = 0;
    area.innerHTML = `
            <div class="project-details-panel">
                <h3>${project.title}</h3>
                <ul class="pd-meta">
                    ${metaItem('fa-regular fa-calendar', t.duration, d.duration)}
                    ${metaItem('fa-solid fa-user-tie', t.role, d.role)}
                    ${metaItem('fa-solid fa-users', t.team, d.team)}
                </ul>
                ${d.description ? `<p class="pd-description">${d.description}</p>` : ''}
                ${d.tools.length ? `
                <h4 class="pd-heading"><i class="fa-solid fa-screwdriver-wrench"></i>${t.tools}</h4>
                <ul class="pd-tools">${d.tools.map(tool => `<li>${tool}</li>`).join('')}</ul>` : ''}
                ${d.responsibilities.length ? `
                <h4 class="pd-heading"><i class="fa-solid fa-list-check"></i>${t.resp}</h4>
                <ul class="pd-list">${d.responsibilities.map(item => `<li>${item}</li>`).join('')}</ul>` : ''}
            </div>`;
    modal.classList.remove('modal-contact', 'modal-cv');
    modal.classList.add('active', 'modal-details');
    area.scrollTop = 0;
  };

  // Modal For CV Quick View (all resume information)
  window.openCVQuickView = () => {
    const source = document.getElementById('cv-data');
    if (!source) return;
    const modal = document.getElementById('master-modal');
    const area = document.getElementById('modal-content-area');
    const panel = document.createElement('div');
    panel.className = 'project-details-panel cv-panel';
    panel.innerHTML = source.innerHTML;
    const projectsList = panel.querySelector('.cv-projects');
    if (projectsList) {
      const projects = (chatFlow['projects'] && chatFlow['projects'][0].data) || [];
      projectsList.innerHTML = projects.filter(pr => pr.details).map(pr => {
        const d = pr.details;
        const sub = [d.role, d.team ? `Team of ${d.team}` : ''].filter(Boolean).join(' · ');
        return `
                <details class="cv-project"${projects.filter(pr => pr.details).indexOf(pr) === 0 ? ' open' : ''}>
                    <summary><span class="cv-project-title">${pr.title}</span><span class="cv-date">${d.duration}</span></summary>
                    <div class="cv-project-body">
                        ${sub ? `<p class="cv-entry-sub">${sub}</p>` : ''}
                        ${d.description ? `<p class="pd-description">${d.description}</p>` : ''}
                        ${d.tools.length ? `<ul class="pd-tools">${d.tools.map(tool => `<li>${tool}</li>`).join('')}</ul>` : ''}
                        ${d.responsibilities.length ? `<ul class="pd-list">${d.responsibilities.map(item => `<li>${item}</li>`).join('')}</ul>` : ''}
                    </div>
                </details>`;
      }).join('');
    }
    ...
    modal.classList.add('active', 'modal-details', 'modal-cv');
    area.scrollTop = 0;
    setQuickViewParam(true);
  };
  ...
  window.closeMasterModal = () => {
    const modal = document.getElementById('master-modal');
    const area = document.getElementById('modal-content-area');
    if (modal.classList.contains('modal-cv')) setQuickViewParam(false);
...
window.addEventListener('load', () => {
  ...
  // 3. Deep link: "?quick_view=true" opens the CV quick view popup
  if (new URLSearchParams(window.location.search).get('quick_view') === 'true') {
    setTimeout(() => window.openCVQuickView(), preloader ? 900 : 0);
  }
